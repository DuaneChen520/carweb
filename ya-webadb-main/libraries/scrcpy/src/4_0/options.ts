...
export class ScrcpyOptions4_0<TVideo extends boolean>
    implements ScrcpyOptions<Init<TVideo>>, ScrcpyOptionsListEncoders
{
    ...
    serialize(): string[] {
        const result = serialize<Init<boolean>>(this.value, Defaults);
        console.log('[DEBUG-4a2f] serialize result:', result);
        console.log('[DEBUG-4a2f] this.value keys:', Object.keys(this.value).sort().join(','));
        console.log('[DEBUG-4a2f] Defaults keys:', Object.keys(Defaults).sort().join(','));
        return result;
    }
...
export namespace ScrcpyOptions4_0 {
